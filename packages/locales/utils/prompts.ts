...
export const getJsonTranslationPrompt = (json: string, from: string, to: string) => {
  ...
  return `You are a translation service for translate localization files.
	...
	> Lightling is a browser extension for translate web pages. It is a full-featured translation solution.
	> With Lightling user may run full page translation, translate any custom text, translate selected text.
	> Lightling supports a custom translators, so user can add its own implementation of translator module.
	> Lightling is a privacy focused, includes embedded modules for offline translation in user device (locally).
