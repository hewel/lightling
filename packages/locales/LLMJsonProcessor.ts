      ...
      Array(this.config.concurrency ?? 3)
        ...
        .map(async () => {
          while (true) {
            ...
            const messages: MessageObject[] = [{ role: 'user', content: prompt(slice) }];
            ...
            for (let retry = 0; true; retry++) {
              ...
              try {
                ...
                const response = await this.fetcher.query(structuredClone(messages));

                // Update messages
                messages.push(...response);

                const transformedSlice = messages.slice(-1)[0].content;
                if (!transformedSlice) throw new TypeError('Empty message in response');
                ...
                try {
                  ...
                  if (onParsingError) {
                    correctionMessages = onParsingError(transformedSlice);
                  }
                  throw error;
                }
                ...
                // Validate slice
                if (validateSlice) {
                  const validationResult = validateSlice(sourceObject, transformedObject);

                  if (!validationResult.isValid) {
                    // Correction messages
                    if (validationResult.correctionRequest) {
                      correctionMessages = validationResult.correctionRequest;
                    }
                ...
                transformedSlices[currentIndex] = Object.entries(transformedObject);
