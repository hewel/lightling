...
command
  ...
  .action(async (sourceLanguage: string, dir: string, rawOptions: unknown) => {
    ...
    const sourceLocaleFilename = path.join(resolvedDir, sourceLanguage, 'messages.json');
        ...
        ),
        {
          ...
          onParsingError(text) {
            ...
            return [
              {
                role: 'user',
                content: 'Your JSON is invalid. Fix it and send me back valid JSON',
              },
            ];
          },
      ...
      try {
        ...
        const syncedLocale = await localesManager.sync({
          source: {
            ...
            previous: sourceLocalePrevRaw ? JSON.parse(sourceLocalePrevRaw) : undefined,
          },
          target: {
