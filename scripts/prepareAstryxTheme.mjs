...
const generatedThemePath = resolve(projectDirectory, 'src/themes/astryx-neutral.css');
const localRequire = createRequire(import.meta.url);
const packageThemePath = localRequire.resolve('@astryxdesign/theme-neutral/theme.css');
const scopeOpening = '@scope ([data-astryx-theme="neutral"]) to ([data-astryx-theme]) {';
const scopeSelector = '[data-astryx-theme="neutral"]';
const generatedHeader = `/*
 * Generated by scripts/prepareAstryxTheme.mjs from
 * @astryxdesign/theme-neutral/theme.css.
 *
 * Extension.js currently removes CSS scope blocks while compiling extension CSS.
 * This compatibility copy expands the neutral scope into descendant selectors
...
export function convertAstryxThemeCss(source) {
	...
	if (scopeCount === 0) {
		throw new Error('No Astryx neutral theme scopes were found');
	}
	...
	if (
		...
		throw new Error('Astryx neutral theme conversion is incomplete');
	}
...
export async function prepareAstryxTheme() {
	const source = await readFile(packageThemePath, 'utf8');
