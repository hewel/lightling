...
export class BergamotTranslator {
  ...
  constructor() {
    const workerUrl = browser.runtime.getURL('thirdparty/bergamot/translator.worker.js');
    const backing = new TranslatorBackingWithCache({
      ...
      onerror(err) {
        console.warn('BergamotTranslator: error in TranslatorBackingWithCache', err);
      },
    });
  ...
  translate = async (text: string, from: string, to: string) => {
    if (from === 'auto') {
      ...
      if (detectedLanguage !== null && langs.includes(detectedLanguage)) {
        ...
        const defaultLang = to !== 'en' ? 'en' : langs.find((lang) => lang !== to);
        from = defaultLang ?? 'en';
      }
    }
