...
describe('data migration util', () => {
  ...
  test('migrate from v0 to v4', async () => {
    ...
    expect(data.length).toBe(3);
    data.forEach((data) => {
      expect(Object.keys(data).sort()).toEqual(['name', 'keywords', 'hashtags'].sort());
    });
  });
  ...
  test('migrate from v1 to v4', async () => {
    ...
    expect(data.length).toBe(3);
    data.forEach((data) => {
      expect(Object.keys(data).sort()).toEqual(['title', 'keywords', 'hashtags'].sort());
    });
  });
  ...
  test('migrations with previous version condition from v0 to v3', async () => {
    ...
    expect(data.length).toBe(3);
    data.forEach((data) => {
      expect(Object.keys(data).sort()).toEqual(['name', 'tags', 'additionalData'].sort());
    });
  });
});
