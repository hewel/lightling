...
export const TranslatorsManager: FC<{
  ...
  const [editedTranslator, setEditedTranslator] = useState<CustomTranslator | null>(null);
  ...
  const deleteTranslatorWithConfirmation = useCallback(
    (translator: CustomTranslator) => {
      const isConfirmed = confirm(
        getMessage('translatorsManagerWindow_message_translatorRemovingConfirmation', [
          translator.name,
        ]),
      );
        ...
        <div {...stylex.props(styles.root)}>
          ...
          >
                ...
                return (
                  <div {...stylex.props(styles.entry)} key={id}>
                    ...
                    <HStack gap={2}>
                      ...
                      >
                        {getMessage('translatorsManagerWindow_translator_edit')}
                      </Button>
                      <Button
                        onPress={() => {
                          deleteTranslatorWithConfirmation(translatorInfo);
                        }}
                        title={getMessage('translatorsManagerWindow_translator_delete')}
                      >
                        <IconTrash />
                      </Button>
                    </HStack>
            ...
            {translators.length !== 0
              ? undefined
              : getMessage('translatorsManagerWindow_emptyTranslatorsListText')}
          </ModalLayout>
        </div>
      )}
