...
export const getTranslatePreferencesForSite = (
  ...
  if (sitePreferences !== null) {
    ...
    if (!sitePreferences.enableAutoTranslate) {
      ...
      const isAutoTranslatedLang = sitePreferences.autoTranslateLanguages.includes(lang);
      const isIgnoredLang = sitePreferences.autoTranslateIgnoreLanguages.includes(lang);
